...
class FileRenameApp:
    ...
    def __init__(self, root):
        ...
        """
        self.root = root
        self.root.title(f"📝 파일명 정규화 도구 v{__version__}")
            ...
            for filename in sorted(files):
                ...
                if not has_genre:
                    ...
                    self.genre_file_items.append([
                        ...
                        None,           # 2: 추론된 장르 (이미 있음)
                        'high',         # 3: 신뢰도
                        False           # 4: 체크 여부 (장르 있으면 False)
                    ])
            
            # 기본 정렬: 체크된 항목 먼저, 그 다음 파일명 오름차순
            self.genre_file_items.sort(key=lambda x: (not x[4], x[1].lower()))
            self.genre_sort_column = 'check'
            self.genre_sort_reverse = False
            
            self.update_genre_tree()
            self.update_genre_sort_indicators()
            self.status_label.config(text=f"🎭 총 {len(self.genre_file_items)}개 파일 (장르 필요: {needs_genre_count}개)")
    ...
    def update_genre_tree(self):
        ...
        for idx, item in enumerate(self.genre_file_items):
            ...
            # 장르가 이미 있는 파일 처리
            if item[2] is None and not item[4]:
                genre_str = "이미 있음"
                confidence_str = "-"
            else:
            ...
            if item[2] is None and not item[4]:
                # 장르가 이미 있는 파일 (회색)
                self.genre_tree.item(item_id, tags=('has_genre',))
            elif item[3] == 'medium':
        ...
        # 태그 색상 설정
        self.genre_tree.tag_configure('has_genre', background='#F5F5F5')          # 회색 (이미 장르 있음)
    ...
    def sort_genre_by_column(self, column):
        ...
        if column == 'check':
            # 체크 여부로 정렬
            # False=0, True=1이므로 reverse=False면 체크 안 된 것(0)이 먼저
            # 체크된 항목을 먼저 보려면 reverse=True로 정렬
    ...
    def execute_add_genre(self):
        """장르 추가 탭: 장르 추가 실행"""
        # 체크되고 장르가 있는 항목만 필터링
        checked_items = [item for item in self.genre_file_items if item[4] and item[2] is not None]
        
        if not checked_items:
            messagebox.showwarning("경고", "선택된 파일이 없거나 장르가 지정되지 않았습니다")
            return
    ...
    def load_files(self):
        """
        폴더에서 파일 목록을 읽어와 정규화 (탭 1 - 정규화 전용)
        
        지원 확장자: .txt, .epub, .zip, .zipx, .7z, .rar
        모든 파일을 목록에 추가하되, 변환이 필요한 파일만 체크 표시합니다.
        
        주의: 장르 자동 추론 기능은 제거되었습니다.
        장르는 탭 2에서 추가할 수 있습니다.
        """
        ...
        try:
            ...
            for filename in sorted(files):
                ...
                if normalized:
                    # 원본과 다른 경우 변환 필요
                    needs_conversion = (normalized != filename)
                    if needs_conversion:
                    ...
                    self.file_items.append([
                        ...
                        needs_conversion,  # 3: 체크 여부 (변환 필요한 경우만 True)
                        normalized,     # 4: 사용자가 수정한 파일명
                        False,          # 5: 사용자 확인 필요 여부 (항상 False)
                        'high'          # 6: 신뢰도 (항상 high)
                    ])
                else:
                    # 정규화 실패한 경우도 표시 (체크 해제)
                    self.file_items.append([
                        full_path,      # 0: 원본 전체 경로
                        filename,       # 1: 원본 파일명
                        filename,       # 2: 정규화된 파일명 (원본과 동일)
                        False,          # 3: 체크 여부 (False)
                        filename,       # 4: 사용자가 수정한 파일명
                        False,          # 5: 사용자 확인 필요 여부
                        'high'          # 6: 신뢰도
                    ])
            ...
            self.status_label.config(text=f"📝 총 {len(self.file_items)}개 파일 (변환 필요: {needs_conversion_count}개)")
    ...
    def update_tree(self):
        ...
        for idx, item in enumerate(self.file_items):
            ...
            item_id = self.tree.insert('', 'end', iid=str(idx), 
                           values=(check_mark, item[1], item[4]))
            
            # 변환이 필요 없는 파일 (원본과 동일) - 회색 배경
            if item[1] == item[4]:
                self.tree.item(item_id, tags=('no_change',))
        
        # 태그 색상 설정
        self.tree.tag_configure('no_change', background='#F5F5F5')  # 회색 (변환 불필요)
