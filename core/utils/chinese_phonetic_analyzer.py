"""
==============================================================================
파일: core/utils/chinese_phonetic_analyzer.py
역할 및 목적:
    중국 웹소설(치뎬, 진장 등)이 기계 번역 또는 한국식 한자음(음독/음차)으로 직역된
    특유의 파일명 패턴을 감지하고, 어휘 구조와 클리셰를 정밀 분석하여
    올바른 장르(현판, 선협, 무협, 판타지, 겜판, 언정 등)를 추론하는 전용 분석기.
주요 구성 요소:
    - ChinesePhoneticResult: 분석 결과 데이터클래스 (genre, confidence, matched_pattern, reason)
    - ChinesePhoneticAnalyzer: 음독/번역투 패턴 분석 엔진 클래스
==============================================================================
"""
from dataclasses import dataclass
from typing import Optional, List, Dict, Tuple
import re


@dataclass
class ChinesePhoneticResult:
    """중국 소설 음독/번역투 분석 결과"""
    genre: str = "미분류"
    confidence: str = "none"         # 'high', 'medium', 'low', 'none'
    matched_pattern: str = ""
    reason: str = ""
    is_detected: bool = False


class ChinesePhoneticAnalyzer:
    """중국 웹소설 번역투 및 음독 제목 정밀 분석기"""

    # 1. 대표적인 중국어 문장형 제목 음독/직역 접두사 (Regex)
    PREFIX_PATTERNS = [
        # 这个 (Zhège) -> 이석, 저개, 이거
        (r'^(?:이석|저개|이개|이거)', '이석(这个: 이...)'),
        # 我真没想 (Wǒ zhēn méi xiǎng) -> 이신몰상, 아진몰상, 아몰상, 아진불상
        (r'^(?:이신몰상|아진몰상|아몰상|아진불상|이진몰상)', '이신몰상/아진몰상(我真没想: 내가 진짜 ~할 생각은 없었는데)'),
        # 我的 (Wǒ de) -> 이적, 아적
        (r'^(?:이적|아적)', '이적/아적(我的: 나의...)'),
        # 这就是 / 我就是 (Zhè jiùshì / Wǒ jiùshì) -> 이취시, 저취시, 아취시
        (r'^(?:이취시|저취시|아취시)', '이취시/아취시(这就是/我就是: 이것이 바로/내가 바로...)'),
        # 穿成 / 穿书 (Chuānchéng / Chuānshū) -> 천성, 천서, 천서후
        (r'^(?:천성|천서후|천서)', '천성/천서(穿成/穿书: ~로 빙의하다/책빙의...)'),
        # 别惹 (Biérě) -> 별야
        (r'^(?:별야)', '별야(别惹: ~를 건드리지 마라)'),
        # 全家 (Quánjiā) -> 전가
        (r'^(?:전가)', '전가(全家: 온 가족이...)'),
        # 我太想 (Wǒ tài xiǎng) -> 아태상, 아태
        (r'^(?:아태상|아태)', '아태상(我太想: 내가 너무...)'),
        # 我被迫 (Wǒ bèipò) -> 아피박
        (r'^(?:아피박)', '아피박(我被迫: 내가 억지로/강제로...)'),
        # 我合法 (Wǒ héfǎ) -> 아합법
        (r'^(?:아합법)', '아합법(我合法: 내가 합법적으로...)'),
        # 我成为 (Wǒ chéngwéi) -> 아성위, 아하위
        (r'^(?:아성위|아하위)', '아성위(我成为: 내가 ~가 된...)'),
        # 从...开始 (Cóng... kāishǐ) -> 종...개시, ...개시
        (r'^(?:종.+?개시|개시)', '종~개시(从~开始: ~부터 시작하다)'),
        # 开局 (Kāijú) -> 개국
        (r'^(?:개국)', '개국(开局: 시작부터...)'),
        # 연대물 접두사: 1968, 1979, 1983, 60년대, 70년대, 80년대, 칠령, 육령, 팔령
        (r'^(?:19[5-9]\d|[5-9]0년대|칠령|육령|팔령)', '연대물(1960~80년대/칠령/육령/팔령)'),
        # 嫁给 (Jià gěi) -> 가급
        (r'^(?:가급)', '가급(嫁给: ~에게 시집가다)'),
        # 家有 (Jiā yǒu) -> 가유
        (r'^(?:가유)', '가유(家有: 집에 ~가 있다)'),
        # 开着外挂 (Kāizhe wàiguà) -> 개착외괘
        (r'^(?:개착외괘|개착)', '개착외괘(开着外挂: 치트를 켜고)'),
        # 降星 / 强盛 -> 강성
        (r'^(?:강성선신|강성)', '강성(强盛/降星)'),
        # 妻子 / 娇妻 (Qīzi / Jiāoqī) -> 악자, 교처
        (r'^(?:악자|교처)', '악자/교처(妻子/娇妻: 아내/애교아내...)'),
    ]

    # 2. 음독 및 특수 어휘 기반 장르 매핑 규칙
    # (키워드 목록, 장르, 우선순위 가중치, 설명)
    TRAIT_RULES: List[Tuple[List[str], str, float, str]] = [
        # 패러디 (최고 우선순위: 나루토, 원피스, 블리치, 주술회전, 서브컬처)
        (
            [
                '호카게', '목엽', '화영', '사륜안', '윤회안', '나루토', '차크라',
                '열매', '악마의 열매', '물고기물고기', '고무고무', '원피스', '해군', '해적',
                '참백도', '만해', '블리치', '호정13대', '주술회전', '영역전개', '특급주령',
                '고죠', '스쿠나', '주력', '마인크래프트', '포켓몬', '악타입', '사천왕',
                '호그와트', '해리포터', '슬리데린', '그리핀도르', '홈랜더', '모리어티',
                '엘든링', '애이등법환', '두파', '소훈아', '투라대륙', '무혼', '라삼포',
                '류이룡', '빙여화', '하치만', '내청코', '탄서성공', '새마낭', '우마무스메',
                '천룡인', '키자루', '제넨사', '호흡법', '두라', '두라지', '斗罗',
                '인계', '닌계', '키사메', '간시귀', '부주인료', '하지메', '원펀맨',
                '새로운 흑황제', '흑황제의 강림', '신비의 제왕', '치신세계', '궤비：', '궤비지주',
                '베이커가', '베이커', '사신은 순애', '인재탄서', '탄서'
            ],
            '패러디', 0.98, '애니/게임/서브컬처 패러디 고유 어휘'
        ),
        # 스포츠
        (
            [
                '발롱도르', '스트라이커', '골키퍼', '미드필더', '공격수', '득점왕',
                '좌완파이어볼러', '파이어볼러', '야구', '투수', '홈런', '축구'
            ],
            '스포츠', 0.98, '스포츠(축구/야구 등) 고유 어휘'
        ),
        # 선협 / 수선 (최고 우선순위 고유 장르)
        (
            [
                '수선', '修仙', '수진', '修真', '선협', '仙侠', '마두', '魔头',
                '대승기', '大乘期', '비승', '飞升', '도우', '道友', '선종', '종문',
                '축기', '금단', '원영', '화신', '연허', '합체', '천겁', '도겁',
                '선존', '선인', '단약', '영초', '영맥', '영근', '구도', '苟到',
                '불안투로', '척패', '자귀', '선신', '강성선신', '검단',
                '광음지외', '구마', '선역', '무동건곤', '심공피안', '아사형실재태온건료',
                '대겁주', '선옥', '도가선자', '참요무성', '헌제성신', '수설저정류전',
                '흑백무제', '군성지자도혼록', '구신지전', '망장천', '선마녀',
                '대황수야인', '노조', '홍황', '봉신', '로조', '老祖', '선경', '仙境'
            ],
            '선협', 0.95, '선협/수선 고유 어휘'
        ),
        # 공포 / 괴담
        (
            [
                '444번 병원', '444호', '괴담', '괴이', '흉가', '악령', '퇴마', '오컬트',
                '괴이관리국', '미제사건', '동경괴담', '폐가'
            ],
            '공포', 0.95, '공포/괴담/오컬트 어휘'
        ),
        # 여성향 언정 (단총, 교처, 궁투, 택투, 리혼, 연대물 등)
        (
            [
                '단총', '甜宠', '교처', '娇妻', '리혼', '수부', '首富',
                '궁투', '宫斗', '택투', '宅斗', '복보', '福宝', '진천금', '가천금',
                '포태', '소내포', '시집', '시어머니', '부군', '낭자', '규수', '모친',
                '포회', '炮灰', '전처', '前妻', '천성', '穿成', '천서', '穿书',
                '천서후', '대도황', '逃荒', '도황', '대노', '大佬', '제전',
                '여배', '반파', '년대문', '녹차녀', '만급녹차', '도혼', '맹보',
                '소농녀', '지청', '천억 물자', '억만 물자', '적장녀', '명문장녀',
                '서장자', '후문독비', '재입후문', '첩신아환', '아낭사가', '녀제',
                '후비', '독비', '아기님', '편집태자', '사둔후폐하', '울어봐 빌어도 좋고',
                '녀주', '년대고정', '1v2', '수라장', '중생후왕비', '왕비함어', '소복녀', '농가소복녀', '함어료'
            ],
            '언정', 0.95, '중국 여성향(언정)/연대물 고유 클리셰'
        ),
        # 게임판타지 (생존유희, NPC, 유희 등)
        (
            [
                '생존유희', '生存游戏', 'NPC', '가상현실', '유희', '游戏', '로그인',
                '히든클래스', '전직클래스', '퀘스트', '인던', '던전', '레벨업', '공략', '플레이어',
                '속성반', '속성판', '마투', '치트모드', '히든피스', '공로구생', '도생', '랭커'
            ],
            '겜판', 0.90, '게임판타지/생존게임 클리셰'
        ),
        # 현대판타지: 말세 / 아포칼립스 / 좀비 / 생존
        (
            [
                '말세', '末世', '아포칼립스', '좀비', '생존', '대재앙', '빙하기',
                '멸망', '괴수', '변이', '돌연변이', '안식처', '피난처'
            ],
            '현판', 0.92, '말세(아포칼립스) 현대판타지'
        ),
        # 퓨전판타지: 스팀펑크 / SF / 재변 / 회빙환
        (
            [
                '스팀펑크', '말일', '말일락원', '증기붕극', '유토피아', '복활전인류', '재변', '제1서열', '특이점',
                '회빙환', '출롱기'
            ],
            '퓨판', 0.92, '스팀펑크/SF/퓨전판타지'
        ),
        # 현대판타지: 무한류 / 복제 / 시스템 / 이능력 / 연예계 / 직업물
        (
            [
                '무한 복제', '복제', '무한', '무한류', '스킬복사', '능력복사',
                '어신', '어장', '계통', '系统', '골드핑거', '사인', '출석체크',
                '역습', '모의기', '인생시뮬', '시뮬레이터', '신호', '재벌', '스타',
                '아이돌', '연예인', '감독', '배우', '상인', '호래오', '할리우드',
                '오락시대', '만화대사', '판사', '래퍼', '요리', '알바생', '생화학자',
                '스트리머', '회장님', '보디가드', '심부름센터', '디자이너', '작곡천재',
                '공무원', '의원님', '건스미스', '파일럿', '미전실', '먹방', '야쿠자',
                '이능자', '학패', '연기', '사원', '회사원', '자취',
                '생활계', '생활계직업', '속성점', '일근육', '인재동경'
            ],
            '현판', 0.88, '현대판타지/전문직/연예계 클리셰'
        ),
        # 판타지: 서양 판타지 / 악마 / 감옥 / 마왕 / 판타지
        (
            [
                '세모', '恶魔', '비아니스', '빌아니시', '마갑', '엑스트라 지갑송',
                '방개나개녀무', '비륜대륙', '권왕마녀', '스켈레톤',
                '세계수', '현환', '타람', '숲의 종족', '무사', '巫师'
            ],
            '판타지', 0.88, '중국 서양 판타지/음독 클리셰'
        ),
        # 무협: 고전 무협 / 난세서 / 강호 / 문파 / 객잔
        (
            [
                '난세서', '난세', '강호', '무림', '검황', '검성', '무신', '협객',
                '검협', '도협', '풍운', '비급', '소림', '무당', '화산', '마교',
                '당문', '세가', '악귀나찰', '무인 이곽', '이곽', '일대종사', '멸문',
                '자객', '련무태난', '합성계무사', '북산철벽', '신마경천기', '천하를 쥐다',
                '객잔', '소호객잔', '군림', '언가군림', '인주란', '항마신장', '항마장'
            ],
            '무협', 0.92, '전통 무협/강호 클리셰'
        ),
        # 역사
        (
            [
                '초한지', '만당', '과거', '위관', '출사', '흥가', '공명로', '관군신조',
                '국사무쌍', '국자감', '민국', '북송', '촉한', '청천', '탐화',
                '대명', '大明', '주원장', '진시황', '秦始皇', '개국시조'
            ],
            '역사', 0.90, '역사/대체역사/고전 클리셰'
        ),
        # 패러디 / 2차 창작 (종만, 두라/투라, 목엽, 망왕, 사신 등)
        (
            [
                '종만', '综漫', '두라', '투라', '斗罗', '목엽', '木叶', '닌자', '忍界',
                '망왕', '网王', '포켓몬', '동인', '패러디', '2차창작', '사신생활',
                '사신은 순애', '사신'
            ],
            '패러디', 0.95, '중국 2차 창작/동인/종만 패러디 클리셰'
        ),
    ]

    # 특정 유명 작품 직접 매핑
    KNOWN_TITLES: Dict[str, Tuple[str, str]] = {
        '난세서': ('무협', '희차 작가 명작 무협 소설 《乱世书》'),
        '이신몰상중생이': ('현판', '치뎬 메가히트 현대 일상/경영 회귀 소설 《我真没想重生啊》'),
        '아진몰상중생아': ('현판', '치뎬 메가히트 현대 일상/경영 회귀 소설 《我真没想重生啊》'),
        '아태상중생료': ('현판', '치뎬 현대 도시 회귀 소설 《我太想重生了》'),
        '광음지외': ('선협', '이천근 작가 대표 선협 소설 《光阴之外》'),
        '구마': ('선협', '이천근 작가 3대 선협 소설 《求魔》'),
        '선역': ('선협', '이천근 작가 대표 선협 소설 《仙逆》'),
        '무동건곤': ('선협', '천치에두 작가 대표 선협 소설 《武动乾坤》'),
        '심공피안': ('선협', '진동 작가 대표작 《深空彼岸》'),
        '아사형실재태온건료': ('선협', '선협 명작 소설 《我师兄实在太稳健了》'),
        '대겁주': ('선협', '선협 명작 소설 《大劫主》'),
        '제1서열': ('퓨판', '회화적삼도 작가 아포칼립스 명작 《第一序列》'),
        '방개나개녀무': ('판타지', '서양 판타지 명작 《放开那个女巫》'),
        '빌아니시 귀': ('판타지', '판타지/코노스바 악마 패러디 계열 소설'),
        '빌아니시': ('판타지', '판타지/코노스바 악마 패러디 계열 소설'),
        '출롱기': ('퓨판', '포스트 아포칼립스 SF 소설 《出笼记》'),
        '검주편봉적대명': ('역사', '명나라 역사 대체 소설 《剑走偏锋的大明》'),
        '소호객잔': ('무협', '전통 무협 객잔 소설'),
        '공룡시대의 판타지': ('판타지', '판타지 소설'),
        '나만, 연기가 너무 쉽다': ('현판', '배우/연기 현대판타지 소설'),
        '천재 사원이 되었습니다': ('현판', '회사/직장 현대판타지 소설'),
        '천재 살아가는 법': ('현판', '웹소설 작가 현대판타지 소설'),
        '자취만 하는데 너무 강함': ('현판', '현대 일상/능력물 소설'),
        '일식이가 간다': ('현판', '현대 회귀 소설'),
        '엄마를 헬스장에 보내지 마': ('현대', '현대 코미디 일상 소설'),
        '이웃집 섹트녀': ('현대', '현대 성인 소설'),
        '어떤 여자든 착정생물로 세뇌 개조하는 가면을 주웠다': ('판타지', '판타지 라이트노벨 소설'),
        '인간 자체가 강함 개정판': ('퓨판', '헌터/퓨전판타지 소설'),
        '인간 자체가 강함': ('퓨판', '헌터/퓨전판타지 소설'),
        '어둠의 군주에서 어둠의 군주까지': ('판타지', '판타지 소설'),
        '빌런의 여동생이다': ('판타지', '판타지 소설'),
        '회빙환 같은데 장르를 짐작할 수 없다': ('퓨판', '퓨전판타지 소설'),
        '지요타포？ 나아니？': ('언정', '로맨스/언정 소설 《只要他抱？那我呢？》'),
        '년대고정녀주간도탄막후': ('언정', '연대물 언정 소설 《年代古穿今女主看到弹幕后》'),
        '년대：아가후원시만계랄급장': ('현판', '연대물 현대판타지 소설'),
        # 수정요함 및 해외 주요작
        '시간을 가르는 나의 정체성': ('현판', '신비의 제왕 동인/현대 초월자 소설'),
        '수많은 세계, 쉐임리스부터 시작한다': ('현판', '치뎬 제천무한/시스템 소설 《诸天从无耻之徒开始》'),
        '새로운 흑황제의 강림': ('패러디', '신비의 제왕 흑황제 동인 패러디 소설 《新黑皇帝的降临》'),
        '치신세계': ('패러디', '신비의 제왕 동인 패러디 소설 《诡秘：致新世界》'),
        '궤비：치신세계': ('패러디', '신비의 제왕 동인 패러디 소설 《诡秘：致新世界》'),
        '촉한지장가한': ('역사', '치뎬 삼국지 역사 소설 《蜀汉之庄稼汉》'),
        '중생후왕비함어료': ('언정', '진장문학성 고대 언정 로맨스 소설 《重生后王妃咸鱼了》'),
        '웨코문드에서부터 시작하는 사신생활': ('패러디', '블리치 2차 창작 패러디 소설'),
        '구미의 밤, 뇌광만겁': ('패러디', '나루토 2차 창작 패러디 소설'),
        '짐취시만력제': ('역사', '명나라 만력제 역사 대체 소설 《朕就是万历帝》'),
        '천도수근, 종살저장련성인간무성': ('무협', '무성 연마 무협 소설 《天道酬勤，从杀猪匠练成人间武圣》'),
        '중회구령고사업': ('현판', '90년대 창업 회귀 소설 《重回九零搞事业》'),
        '집장오뢰행량계': ('선협', '오뢰 뇌법 선협 소설 《执掌五雷行两界》'),
        '중계신화': ('선협', '신화 리부트 선협 소설 《重启神话》'),
        '지척지간인진적국': ('현판', '승풍어필 작가 고무 도시 소설 《咫尺之间人尽敌国》'),
        '장등판관': ('현판', '도시 판관 오컬트 소설 《掌灯判官》'),
        '농가소복녀': ('언정', '중국 고대 농가 언정 소설 《农家小福女》'),
        '대치동 클래스': ('현판', '현대 판타지 학원/전문가 소설'),
        '멸문한 가문의 서자가 돌아왔다': ('퓨판', '퓨전 판타지 귀환 소설'),
        '고인물 스트라이커는 미쳤다': ('스포츠', '현대 축구 스포츠 소설'),
        '이혼했더니 발롱도르를 수상했다': ('스포츠', '현대 축구 스포츠 소설'),
        '다차원 파견 회사': ('현판', '현대 차원 판타지 소설'),
        '항마신장': ('무협', '전통 무협 소설'),
        '저유희야태진실료': ('겜판', '치뎬 가상현실 폐토 게임 소설 《这游戏也太真实了》'),
        '시작부터 구미호가 마리조아를 포격하다': ('패러디', '원피스 2차 창작 패러디 소설 《开局九尾炮轰玛丽乔亚》'),
        '중생지내파적유한생활': ('현판', '치뎬 현대 도시 육아/일상 힐링 소설 《重生之奶爸的悠闲生活》'),
    }

    @classmethod
    def analyze(cls, text: str, pure_title: str = "") -> ChinesePhoneticResult:
        """
        중국 웹소설 기계번역/음독 제목을 분석하여 장르 판정
        
        Args:
            text: 원본 파일명 또는 파싱된 텍스트
            pure_title: 순수 제목 (옵션)
            
        Returns:
            ChinesePhoneticResult
        """
        result = ChinesePhoneticResult()
        
        # 1. 제목 정제
        target_title = (pure_title or text).strip()
        # 확장자 및 회차 제거
        cleaned = re.sub(r'\.[a-zA-Z0-9]+$', '', target_title)
        cleaned = re.sub(r'[\(\[\{].*?[\)\]\}]', '', cleaned).strip()
        cleaned = re.sub(r'\s*\d+[-~]\d+.*$', '', cleaned).strip()
        cleaned = re.sub(r'\s*\(완\).*$', '', cleaned).strip()

        # 2. 유명 작품 DB 직접 매칭
        for known_k, (genre, reason) in cls.KNOWN_TITLES.items():
            if known_k in target_title or known_k in cleaned:
                result.genre = genre
                result.confidence = "high"
                result.matched_pattern = known_k
                result.reason = reason
                result.is_detected = True
                return result

        # 3. 중국어 음독 접두사 감지
        has_prefix = False
        prefix_desc = ""
        for pat, desc in cls.PREFIX_PATTERNS:
            if re.search(pat, cleaned):
                has_prefix = True
                prefix_desc = desc
                break

        full_context = f"{text} {pure_title} {cleaned}".strip()

        # 4. 특별 클리셰: 사합원(四合院) 판정
        if any(kw in full_context for kw in ['사합원', '四合院', '4합원']):
            female_kws = ['여주', '교처', '낭자', '단총', '복보', '천금', '궁투', '택투', '시어머니', '시집', '포태', '리혼']
            is_female = any(fk in full_context for fk in female_kws)
            result.genre = '언정' if is_female else '현판'
            result.confidence = 'high'
            result.matched_pattern = '사합원'
            result.reason = f"중국 시대극 사합원물 ({'여성향 언정' if is_female else '남성향 현대연대물'})"
            result.is_detected = True
            return result

        # 5. 특별 클리셰: 중생(重生 - 회귀/환생) 문맥 분기
        # '중생'은 남성향 도시물과 여성향 언정물 양쪽에서 가장 흔한 클리셰임!
        if any(kw in full_context for kw in ['중생', '重生']):
            female_kws = ['교처', '단총', '낭자', '궁투', '택투', '여주', '리혼', '복보', '천금', '시어머니', '시집', '포태', '왕비', '함어', '소복녀']
            has_female = any(fk in full_context for fk in female_kws)
            if has_female:
                result.genre = '언정'
                result.confidence = 'high'
                result.matched_pattern = '중생 + 여성향'
                result.reason = '중국 여성향 회귀 로맨스(언정)'
                result.is_detected = True
                return result
            else:
                # 남성향 도시/경영/일상/선협 중생
                if any(sk in full_context for sk in ['수선', '수진', '선협', '마두', '구도']):
                    result.genre = '선협'
                    result.reason = '수선 중생(선협 회귀물)'
                else:
                    result.genre = '현판'
                    result.reason = '도시/현대 중생(회귀물)'
                result.confidence = 'high' if has_prefix else 'medium'
                result.matched_pattern = '중생 (남성향/도시)'
                result.is_detected = True
                return result

        # 6. 특화 규칙 테이블 대조 (우선순위 순)
        for keywords, genre, conf_score, desc in cls.TRAIT_RULES:
            for kw in keywords:
                if kw in full_context:
                    result.genre = genre
                    result.confidence = "high" if (has_prefix or conf_score >= 0.92) else "medium"
                    result.matched_pattern = kw
                    result.reason = f"{desc} ('{kw}' 매칭{f', 접두사: {prefix_desc}' if has_prefix else ''})"
                    result.is_detected = True
                    return result

        # 7. 접두사만 매칭되고 세부 키워드는 없을 때의 기본 추론
        if has_prefix:
            # 이석(这个) / 이신몰상(我真没想) / 이취시(这就是) 등은 기본적으로 치뎬 남성향 현판/판타지가 대다수
            result.genre = "현판"
            result.confidence = "medium"
            result.matched_pattern = prefix_desc
            result.reason = f"중국 소설 음독 접두사({prefix_desc}) 감지, 현대/퓨전 기본 판정"
            result.is_detected = True
            return result

        return result
