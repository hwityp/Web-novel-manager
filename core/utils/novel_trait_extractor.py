...
TRAIT_PATTERNS: List[Tuple[str, List[str]]] = [
    ...
    ("시스템", [
        ...
        r"수많은\s*세계.*시작", r"쉐임리스부터\s*시작한다", r"诸天从.*开始"
    ]),
    ...
    ("궁정", [r"궁정", r"궁궐", r"황궁", r"왕궁", r"조정", r"귀비", r"태의", r"군성귀위자", r"宫廷", r"皇宫"]),
    # 7. 재테크
    ...
    ("원피스", [r"원피스", r"루피", r"조로", r"해적왕", r"마리조아", r"천룡인", r"해군본부"]),
    ("나루토", [r"나루토", r"사스케", r"닌자", r"나뭇잎마을", r"구미의\s*밤", r"호카게", r"목엽", r"木叶", r"인계", r"忍界"]),
    ("투라대륙", [r"두라", r"투라", r"투라대륙", r"斗罗", r"혼환", r"魂环"]),
    ("블리치", [r"블리치", r"참백도", r"사신", r"호로", r"웨코문드", r"이치고"]),
    ...
    ("삼국지", [
        r"삼국지", r"삼국", r"촉한", r"조조", r"유비", r"제갈량", r"여포", r"관우", r"장비", r"동탁", r"三国", r"蜀汉"
    ]),
    ("미드", [
    ...
    def extract_traits(
        ...
        if "시스템" not in selected_traits and "시스템" != primary_genre:
            system_check_patterns = [
                ...
                r"모의기", r"模拟器", r"시뮬레이터", r"인생시뮬", r"골드핑거", r"金手指",
                r"사인", r"签到", r"역습계통", r"수많은\s*세계.*시작", r"쉐임리스부터\s*시작한다", r"诸天从.*开始"
            ]
            if any(re.search(pat, combined_text, re.IGNORECASE) for pat in system_check_patterns):
                if len(selected_traits) >= 2:
                    ...
                    elif "미드" in selected_traits:
                        selected_traits = ["미드", "시스템"]
                    else:
                        selected_traits = [selected_traits[0], "시스템"]
                else:
                    selected_traits.append("시스템")

        if "미드" in selected_traits and "시스템" in selected_traits:
            selected_traits = ["미드", "시스템"]
