"""
==============================================================================
파일: core/utils/similarity.py
역할 및 목적:
    검색 엔진 결과에서 크롤링된 도서명과 원본 파일명에서 추출된 핵심 제목 간의 유사도를 검증하는 모듈.
    레벤슈타인 거리(Levenshtein Distance) 알고리즘 기반으로 유사도(0.0~1.0)를 산출하여,
    엉뚱한 다른 작품의 장르로 오분류되는 현상을 원천 방지합니다.
주요 구성 요소:
    - TitleSimilarityChecker: 유사도 계산 및 임계값 검증 클래스
    - calculate_similarity(): 두 제목 간의 정규화 유사도 백분율 반환
    - is_similar(): 임계값(기본 85%, 저자 일치 시 75%) 충족 여부 불리언 판정
상호 연관 관계 및 의존성:
    - Caller: core.adapters.genre_classifier_adapter
    - Callee: re
수정 시 주의사항:
    - 띄어쓰기 차이, 특수문자, 대소문자는 유사도 계산 전 정규화 전처리 단계를 거쳐야 합니다.
==============================================================================
"""
import re
