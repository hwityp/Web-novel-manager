...
### 규칙 1: Search-First 장르 분류 계층 순서 엄수
장르 분류 시 반드시 아래 순서를 엄격히 준수해야 합니다:
1. **Cache 확인 (Cache-First):** `GenreCache`에 이미 승인된 장르가 있는지 먼저 조회합니다.
2. **웹 검색 (Search-First):** 네이버 검색 API(NaverGenreExtractorV4) → 실패 시 구글 CSE 검색(GoogleGenreExtractor).
3. **플랫폼 가중치 및 제목 유사도 검증:** 검색 결과의 제목과 원본 `task.title`의 유사도(`TitleSimilarityChecker`)가 85% 이상(저자 일치 시 75% 이상)일 때만 채택합니다.
4. **로컬 키워드 폴백 (Keyword Fallback):** 웹 검색에서 유효한 장르를 얻지 못한 경우에만 로컬 키워드 사전(`genre_keywords.json`)으로 분류합니다.

### 규칙 2: Title Anchor 선추출 원칙
